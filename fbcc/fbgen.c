...
void Gen_Cast(LIST *expr)
{
    ...
    switch (t1) {
        ...
        default:
            Error_Internal("type conversion no implemented");
            break;
    }
}
        ...
        case TYPE_UNION:
            ...
            switch(size) {
                case 4:
                    ...
                    break;
                default:
                    Error_Internal("Gen_Assign: struct no implemented");
            }
            break;
        default:
            Error_Internal("Gen_Assign: Type no implemented");
    }
}
...
void Gen_RIdent(LIST *expr)
{
    ...
    switch(t) {
        ...
        case TYPE_ARRAY:
            break;
        default:
            Error_Internal("Gen_RIdent: type undefined");
            break;
    }
}
...
void Gen_Indir(LIST *expr)
{
    ...
    switch(t) {
        ...
        case TYPE_ARRAY:
            break;
        default:
            Error_Internal("Gen_Indir: type undefined");
            break;
    }
}
...
void Gen_LVal(LIST *e)
{
    ...
    switch(tag) {
        ...
        case EXPR_INDIR:
            ...
            break;
        default:
            Error_Internal("Gen_LVal: operation not implemented %x",tag);
            break;
    }
}
...
void Gen_RVal(LIST *e)
{
    ...
    switch(tag) {
        ...
        default:
            Error_Internal("Gen_RVal: %d error",tag);
            break;
    }
}
...
void Gen_InstrBreak(void)
{
    ...
    b=block_current;
    while (1) {
        if (b->type == BLOCK_GLOBAL) Error("Instruction 'break' error");
...
void Gen_InstrContinue(void)
{
    ...
    b=block_current;
    while (1) {
        if (b->type == BLOCK_GLOBAL) Error("Instruction 'continue' error");
...
void Gen_InstrLabel(LIST *ident)
{
    ...
    if (s!=NULL) {
        flag=hd_int(s->list);
        if (flag!=-1) Error("Label '%s' undefined",label);
        l1=hd_int(tl(s->list));
        list_free(s->list);
...
void Gen_InstrCase(LIST *expr)
{
    ...
    b=block_current;
    while (1) {
        if (b->type == BLOCK_GLOBAL) Error("Instruction 'case' not 'default' in switch");
    ...
    if (expr==NULL) {
        /* default */
        if (b->label_restart != -1) Error("default in switch: error");
        b->label_restart=l1;
    } else {
        /* case */
        ...
        b->switch_values=append(b->switch_values, mk_int(hd_int(tl(tl(expr))),mk_int(l1,NULL)));
    }
